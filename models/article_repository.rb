...
class ArticleRepository
  ...
  def self.prepare_statements
    ...
    connection.prepare('insert_article', 'INSERT INTO articles (title, body, created_at) VALUES ($1, $2, NOW()) RETURNING id')
  ...
  def self.add(article)
    result = connection.exec_prepared('insert_article', [article['title'], article['body'] || ''])
    result[0]['id'].to_i
  end
